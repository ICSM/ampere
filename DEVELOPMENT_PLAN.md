...
| Lossless nesting & plate routing | **Approved 2026-09-02** (the R1 follow-ons): `ParameterSet.merge` accepts a `ParameterMapping` as a component and composes its bindings ("lossless, not necessarily recursive" — closes `inference.md` §4.6's introspection defect at source), and `Binding` gains an optional `index` so `distribute` routes one element of a plate's array-valued parameter to one component (`hierarchical_population.md` H-2 — what makes a `Plate` feed per-object datasets). Both amend §4.1's contract; implementation on its own branch. |
