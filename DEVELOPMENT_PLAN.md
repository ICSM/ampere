...
| Non-native parts in a native problem (W2.4 slice 3's carried finding) | **Refused by default; accepted under an explicit opt-in when no gradient is needed; always refused where a gradient is required** (ruled by Peter 2026-09-08 on Fable's recommendation; the §4.5 text lands with W3.8, which amends this row). Today a core numpy `Kernel` inside a torch GP noise model is accepted and its amplitude silently gets no gradient — the same class of silent degradation fold-in 7 removed for solvers and noise models. The default becomes the backend-disagreement refusal, by name, with the remedy. The opt-in exists for the case a native problem cannot express: a piece only expressible in Python (a tabulated kernel, a legacy callback) that a torch/jax problem wants to call through and sample gradient-free. Under the opt-in the affected dataset's `DIFFERENTIABLE` is `False` so the capability ladder tells the truth, the gradient-free engines run and `realise(strict=True)`, `NUTSEngine` and `VIEngine` refuse regardless of the flag; the flag and the foreign parts' names are recorded in provenance. |
