...
- **Jax GP libraries** (Phase 2 gate check): resolved policy is
  ...
  **settled at W2.5 slice 2 (2026-09-07): `QuasisepGP` is celerite2.jax.**
  ...
  rule), and `conditional_loo` still refused (no O(N) route to the
  inverse diagonal, which tinygp *does* offer). The strategy interface is
  what would make a later swap a one-file change.
  ...
  **Both tracks converged on celerite2, by different routes and on different
  ...
  strategy interface is what keeps that a risk rather than a lock-in. The two
  tracks disagree on exactly one thing, and the disagreement is real rather
  than an inconsistency: torch supplies `conditional_loo` in O(N) because it
  calls celerite2's compiled kernels directly and so already holds the
  factorisation, while jax refuses it because `celerite2.jax`'s public surface
  exposes no route to the inverse diagonal. `tests/conformance/README.md` §6
  states that as one debt entry.
