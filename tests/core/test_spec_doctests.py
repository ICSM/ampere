...
W1.3's and W1.4's acceptance criteria are that every worked example in
``docs/design/contracts/parameters.md`` and
``docs/design/contracts/results_schema.md`` runs and passes, so neither spec
can drift from the module it documents without this suite going red. Each
...
@pytest.mark.parametrize("spec", [SPEC, RESULTS_SPEC], ids=["parameters", "results_schema"])
...
@pytest.mark.parametrize(
    "module",
    [ampere.core.parameter, ampere.core.results_schema, ampere.core.exceptions],
    ids=["parameter", "results_schema", "exceptions"],
)
