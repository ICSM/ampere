...
W1.3's, W1.4's and W1.6's acceptance criteria are that every worked example in
``docs/design/contracts/parameters.md``,
``docs/design/contracts/results_schema.md`` and
``docs/design/contracts/likelihoods.md`` runs and passes, so no spec can drift
from the module it documents without this suite going red. Each markdown file is
fed straight to :mod:`doctest`; its examples share one namespace and build on
each other, exactly as a reader would run them.
...
@pytest.mark.parametrize(
    "spec",
    [SPEC, RESULTS_SPEC, LIKELIHOOD_SPEC],
    ids=["parameters", "results_schema", "likelihoods"],
)
...
@pytest.mark.parametrize(
    ...
    ids=["parameter", "results_schema", "likelihood", "exceptions"],
)
