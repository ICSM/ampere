...
| W1.13 | not started — the only remaining Phase 1 item. Peter's 2026-09-03 rulings (see the spec §-preambles and `docs/development.md`) add to its backlog: the accepted-recommendation implementations, the consolidated cross-contract serialisation review, and the overlooked image/1-D-spatial standard-library slots. Cross-model review pairing: Fable + `gpt-5.6-terra` (sol blocked) |
