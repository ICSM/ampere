...
§3's layout gains the `ampere/modalities/` line per D1. (5) `examples/sbi/npe_native.py` — a bare in-process NPE fit of a native
...
wanted 2026-09-10; W3.13's finding). **Depends:** nothing. **Accept:** the two
...
| W3.16 | merged 2026-09-10 at `fea7ef9` (Fable-authored; branch: TMNRE tests 45 passed under the new default in under four minutes, the Poisson rows green, lint/format/pyrefly clean; merged-master gates: MERGED_GATES_W316). `TMNRE_DEFAULT_SAMPLER = "mcmc"`; `PoissonFamily.sample` guards `rate <= 0`; the sampling-form principle in `likelihoods.md` §3; the example's default, its header, the API page and the tutorial updated. Found on the way: `TestTheCalibrationFastPath::test_a_temperature_scaled_posterior_fails_the_same_check` failed once under a `-k` selection that changed the module-scoped fixture order, and passed alone — order-dependent, not a regression (the full gates are the verdict) |
