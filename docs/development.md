...
## ⚡ Pick up here — PHASE 4 IN PREPARATION (2026-09-11): D3–D4 ruled, D1–D2 under discussion on `docs/design/phase4_placement_memo.md`; nothing dispatched yet

**2026-09-11 (Fable, orchestrating session).** Master verified clean at `59454ea` (lint/format/pyrefly clean, import sweep green; the four gates recorded below still stand — every commit since W3.16 is documentation). Peter's rulings on the Phase 4 header: **D3 yes** (W4.9 runs), **D4 agreed** with the 2026-09-05 review-policy ruling extended to Phase 4 (W4.2/W4.5 merge on a second Fable pass while the Codex quota is blocked; terra owed retroactively). **D1 and D2 are not ruled**: Peter rejected the name `modalities` and asked for the placement to be decided from how the existing spectrum/photometry classes behave, legacy included, and for D2's consequences for the problem's dimensionality with wavelength dependence in mind. Three read-only surveys were run and written up as `docs/design/phase4_placement_memo.md` — recommendation: the kind in core beside `VisibilitySet`, the steps in `backends/<backend>/interferometry.py` on all three backends, a top-level `ampere.interferometry` front door only when the first reader (OIFITS) lands, no grouping word; four axes for `ClosurePhases` with a canonical baseline ordering. The memo's §4 lists five corrections to the drafted items found by the surveys (W4.1's `configure_from` claim, W4.3's non-existent step registry, a missing photometry + spectrum example the template page needs, a stale docstring, a stale sketch signature). **A clean session does, in order**: (1) read Peter's D1/D2 rulings (the memo's §6 lists what is asked); (2) apply them to the Phase 4 header and items, with the §4 corrections, and add the small photometry + spectrum example item if approved; (3) then dispatch W4.0 ∥ W4.1 ∥ W4.5 ∥ W4.6 with the standing paragraph and the `core/likelihood.py` ownership split (W4.1 families, W4.5 kernels); (4) refresh this section at the dispatch. The record below is the Phase 3 close as it stood.
