...
## ⚡ Pick up here — PHASE 5 IN FLIGHT; **the wave-2 gate is green in all four environments and recorded (`d71be43`)**; **W5.5 is reviewed and ready — its merge is waiting for Peter** (the permission classifier refuses `git merge` from the session; the rows to paste are below); **W5.13 (Sonnet, `w5.13-population-reweighting`) in flight**; system crash of 2026-09-16 recovered

**State at the end of the recovery session (2026-09-16).** (1) The wave-2 gate (W5.20 + W5.4 + W5.3): dev 2594/407 at `12c6001`, sbi 3625/92, torch 3495/222, jax 3315/231 on `b7a14b2`/`db12df5` — all green, recorded in the three status rows at `d71be43`. (2) **W5.5 is complete on `w5.5-image-psf` at `ff38294`** (six commits, base `9bdba46`, merges clean onto master — verified with `git merge-tree`); Fable's second-pass review read the reference step (`ampere/backends/reference/image.py`), `propagate_mask_grid` and `sample_coordinates`, the torch `problem.py` fixes, the conformance oracle (a true zero-padded convolution with the kernel flipped, sharing nothing with the step), `examples/image/grid_gp.py` and the `transformations.md` amendment: **no fixes needed**. The session's auto-mode permission classifier refused `git merge --no-ff w5.5-image-psf` three times as "Merge Without Review", so **Peter merges** (from the main checkout: `git merge --no-ff w5.5-image-psf`, message in the block below), then pastes the W5.5 status row into WORK_ITEMS.md's table and the three decision-log rows into DEVELOPMENT_PLAN.md §2 (both drafted below, ready), then removes the worktree (`git worktree remove --force --force .claude/worktrees/agent-a955f0a01fed1152b`). (3) **W5.13 reported complete at `f96a092`** (four commits: the module, the netCDF reader, the finished tests — 10 rows, dev 9 passed in 600 s, sbi 10 in 731 s, recovered μ 95 % (0.943, 1.049) and τ (0.285, 0.372) against 1.0/0.3 — and `results.md` §13 **item 16**, not §13.15, since W5.0 took 15) **and was sent back with one review defect**: `_prepare_objects` divides the stored *joint* `log_prior` column out of every draw where the identity needs the named parameter's own marginal interim prior (the nuisance priors cancel), so any object with more than one parameter is biased; the test's `ConstantModel` has one parameter and cannot see it. The agent was told to expose the named parameter's interim log prior through the `RunColumns` protocol (rebuilt from the run's stored `PriorSpec` via `prior_from_spec`/`log_density` if the tree carries it; otherwise an explicit, required prior argument), keep the joint column for the proposal weights, add a two-parameter test against the analytic answer, and re-report. **Review it again on return**; then it goes through the same merge-by-Peter route as W5.5 (rows to be drafted then). (4) **After both merge**: launch the wave-3 gate (all four environments — W5.5 touched core and both backends) as one detached chain, then dispatch **W5.6** (Opus, the bake-off; depends on W5.4 + W5.5) and, in the second slot, **W5.7 ∥ W5.9** per the ordering. (5) The docs-warning note in the operations paragraph is stale: the `Binary`/`Product`/`Sum` warning pairs do not reproduce on `9bdba46` (W5.5's agent rebuilt with `-E` and saw 13 warnings, all legacy).

**Ready to paste at the W5.5 merge.** *Merge message*: "Merge W5.5: a gridded customer — PSFConvolution on the reference path with torch/jax twins, propagate_mask_grid and sample_coordinates in core, the three Layout.GRID fixes on both native paths, the image conformance rows and capability, examples/image with the misspecification study and the DenseGP-vs-HSGP benchmark, docs/source/image.rst, transformations.md §10/§13.5 amended (Opus-authored, Fable-reviewed as the second pass)". *Status row* (fill in the merge hash and, later, the wave-3 gate):

> `| W5.5 | merged 2026-09-16 at <hash> (Opus-authored, Fable-reviewed as the second pass — the FFT route against the direct-sum oracle, the separable dilation in propagate_mask_grid, the C-order grid coordinate matrix in sample_coordinates, the two native-path Layout.GRID fixes and examples/image/grid_gp.py read by hand; terra owed; the original agent was killed by the 2026-09-16 system crash after five commits with its verification and report lost, a fresh Opus agent finished from the branch's HEAD in 45 min with one commit; no fixes needed at review; the agent's runs: tests/conformance/test_image.py dev 28, torch 45, jax 45, sbi 45; full conformance dev 582/65, torch 938/65, jax 938/65; tests/core dev 1197/30; tests/backends dev 147/60, torch (+ test_transform/test_dataset/test_simulate) 986/3, jax 807/11; tests/examples dev 77/9; the image study tests 21/2 (the two image_full rows skipped); lint/format clean, pyrefly 0 errors ×3, import sweep 83/4, docs 13 warnings all legacy; the wave-3 merged-master gate to be recorded with W5.13). Landed: PSFConvolution in each backend's image.py — kernel= tabulated at the observed pixel scale and refused on any other, fwhm= a circular Gaussian rebuilt through context() so promote_buffer fits it, one constraint published in two forms (points= on the padded grid, max_step at the pixel scale), the FFT route requiring an evenly spaced grid and refusing an irregular one by name, from_observed the supported route; the twins inheriting and overriding only the four flags and the sums, grid_steps/crop_indices shared; ampere.core.propagate_mask_grid (a separable dilation by the kernel's bounding half-support, O(kN)); ampere.core.sample_coordinates (layout-aware, used by Dataset.draw_observation and both LoweredProblem.observed_coordinates); both native paths ravel values and uncertainties before the mask and predict() flattens only the container's own trailing axes; tests/conformance/test_image.py with the image capability and ImagePieces; examples/image/ (a compact Gaussian on a broad smooth background; correct/incomplete/flexible arms; SBC calibration; --benchmark DenseGP vs HSGP with --no-dense); docs/source/image.rst; transformations.md §10 PSF row and §13.5 amended; ifu_cube.md gap 2 closed; the image_full marker. Measured: bias in posterior widths correct 0.74/0.45, incomplete 7.92/4.68, flexible 0.78/0.13 (24×24, reference); coverage at nominal 0.9, 12 replicas at 16×16: incomplete [0.00, 0.58], flexible [0.92, 0.92]; benchmark 24×24 DenseGP 0.031 s / 12.7 MiB vs HSGP(8×8) 0.003 s / 0.7 MiB, 64×64 3.925 s / 640 MiB vs 0.006 s / 4.4 MiB, 128×128 HSGP 0.038 s / 17.6 MiB with DenseGP not run (projected ≈10 GiB and ≈4 min — the one Accept number reported by projection). For Peter: the two-edit change in ampere/core/likelihood.py lifting the Layout.GRID refusals (decision-log row), after which grid_gp.py is deleted; confirm --no-dense (the agent's judgement beyond the item text); whether the dense 128×128 cell should be measured on a quiet machine. Carried: python -m examples.image --benchmark allocates ≈10 GiB by default; neither image_full row has been run end to end (≈27 min; ≈10 GiB); tests/examples is not in test-all; a fitted fwhm wandering above the declared width fits a kernel truncated tighter than 5σ, and a negotiated grid finer than the observed one tabulates the Gaussian kernel over a fixed pixel count and so over fewer sigmas (the support is fixed at composition — reviewer's note); the docs warning pairs named in the handoff do not reproduce on this base. |`

*Decision-log rows* (DEVELOPMENT_PLAN.md §2; the agent's wording, checked against the code):

> `| The first gridded observation: PSFConvolution and propagate_mask_grid (W5.5) | A §4 transformation addition and a §13.5 amendment, both within the frozen surface. transformations.md §10's PSF row is filled by a Transformation that publishes one constraint in two forms — points= at the observed pixel centres padded by the kernel's half-support, and intervals/max_step restating the pixel scale as a density — because a PSF, unlike an LSF, has no resampler downstream to land it on the observed grid. The two must agree to the last bit or their negotiated union is not evenly spaced and the FFT route refuses on whichever backend ran, so the padded grid is built the way AxisRequirement.coordinates builds one: if a step publishes the same constraint twice, one form must compute the other. §13.5 limitation 5 ("mask propagation is 1-D") is amended rather than lifted: propagate_mask_grid changes the expression of the ANY rule, not the rule — a local separable kernel's influence is a neighbourhood, carried as a dilation by the kernel's bounding half-support at O(kN) instead of an O(N²) influence matrix — and a grid step whose influence is not local (an arbitrary warp, a non-separable resampling) still flattens its own mask. ifu_cube.md gap 2 closes by the route its freeze disposition named. Conformance green in all four environments. Merged on Fable's second pass; terra owed. |`
> `| Layout.GRID and correlated noise: a gate, not a limit (W5.5) | ampere.core refuses a correlated noise model on every Layout.GRID container (GPSolver.check_compatible at likelihood.py:588 and Likelihood._coordinates at :4047, both "LAYOUT is not Layout.POINTS"), and the refusal is a closed Phase 5 slot rather than mathematics: a stationary kernel over (x, y) is a function of coordinates, and a grid keeps its coordinates separably, so the (N, 2) matrix must be broadcast out of the axes rather than column-stacked — which ampere.core.encoding has done since W3.3 and W5.5 lifted into ampere.core.sample_coordinates. W5.5's ownership put likelihood.py out of scope, so examples/image/grid_gp.py lifts the two gates out of tree from the public API (three subclasses overriding only the layout check), as transformations.md §11 exists to prove is possible. Proposed, for Peter: the library change is two edits in ampere/core/likelihood.py — allow Layout.GRID in GPSolver.check_compatible when the kernel's selected axes are the container's, and build Likelihood._coordinates with sample_coordinates — after which grid_gp.py is deleted and the study imports DenseGP and HilbertSpaceGP directly; it touches likelihoods.md §7 and needs its own row when it lands. Nothing here exploits a grid's structure; that is W5.6's bake-off. |`
