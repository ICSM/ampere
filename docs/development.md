...
## ⚡ Pick up here — PHASE 5 IN FLIGHT since 2026-09-15 (approved with D1–D8 ruled; wave 1 = W5.0 ∥ W5.2 dispatched to two Sonnet agents in harness worktrees, base `adc34e8`; W5.20 next, Opus)
...
**What a clean session does next.** (1) **Wave 1 is in flight** (dispatched 2026-09-15 from base `adc34e8`): W5.0 on branch `w5.0-results-approx-evidence` and W5.2 on `w5.2-housekeeping-owed`, both Sonnet, both in harness worktrees under `.claude/worktrees/` (each installs its own `.pixi`). **W5.0 reported at `b81cd27` (2026-09-16) and was sent back with two defects found at the Fable review by running the branch**: the pyro route read the guide's density off the model site, which is a `Delta` whose `log_prob` is 0, so every stored `proposal_log_density` was just minus the Jacobian term (the importance test passed for the wrong reason); and the numpyro route called `AutoNormal.get_posterior`, which does not exist, so every jax VI run crashed. The fix asked for: sum `log_prob` over every sample site of the traced guide on both routes, tests comparing the stored density against an independently built guide density and asserting it is not constant, and the agent installing the jax environment in its worktree to run the route itself. Everything else in W5.0's diff read correctly (the Jacobian sign in `unconstrained_jacobian_correction` is right: `log q_θ = log q_u − log|∂θ/∂u|`). If this session was cut off, the agents' committed work survives on those branches: `git branch --list 'w5.*'`, `git log <branch> --oneline`; resume an unfinished one from its branch with a fresh agent, or review and merge a finished one (report format in the `work-item` skill; W5.0's report carries its decision-log row text — add it to the plan's §2 at merge, bump the status table, then gate the merged master once in dev + sbi + torch + jax through the `flock`). (2) When one of the two finishes, dispatch **W5.20** (Opus; touches `ampere/core/parameter.py`, both backends' `problem.py` and `torch/parameters.py` — sequence it after W5.2's merge since W5.2 edits both `problem.py`s). (3) Then wave 2 per the ordering in WORK_ITEMS.md "Phase 5". The Phase 5 draft's inputs, for whoever re-reads it:
