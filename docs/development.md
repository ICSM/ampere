...
## ⚡ Pick up here — Phase 3 items DRAFTED 2026-09-08, awaiting Peter's approval; W3.0 in flight
...
**For Peter — the decisions, with where to read.**
1. **Approve, amend or reject the Phase 3 breakdown** (WORK_ITEMS.md "Phase 3"). The load-bearing choices to look at: W3.1 keeps the observation draw on the numpy path and vectorises only the noise-free prediction natively; W3.2 builds sbi's prior in the *unconstrained* coordinates and scores every stored draw on the numpy path so runs carry the true `log_prob`; W3.3 is a new §4 contract (`encoding.md`) with a layout hash; W3.4 (swyft) is gated on a maturity note rather than committed to; W3.5's cache key ingredients; W3.6's placement in `ampere.results.calibration`; the unlimited-dimension append is *deferred* until a budget outgrows memory. Sizes and model tiers are on each heading.
2. **Ratify the "Peter to ratify" decision-log rows** — `DEVELOPMENT_PLAN.md` §2, the rows at lines ~91 (W2.5 slice-2 surface additions), ~92 (RHMF deferral, W2.7), ~94 (results completion, W2.8), ~95 (pytest-benchmark and the docs `-W` deferral, W2.11), ~96–97 (LOO on jax; the gradient-free fast path, W2.5 slice 3), ~98 (the W2.15 annotation batch). Also the Matérn validation note in W2.10's row (WORK_ITEMS.md status table, the W2.10 row).
3. **Open confirmations recorded in status rows** (WORK_ITEMS.md "Status" table): W2.7 — across-draw *median* pooling of the permutation test, `gp_localisation(datasets=None)` meaning every GP dataset, `plot_anomaly_score` returning the Axes; W2.8 — the corner/trace caps (20/40), excluding prior-rejected draws from corner unconditionally, `pointwise_as_log_likelihood` as public API, `"mixed"` in §6's vocabulary; W2.13 fold-in 7's noise-model papercut (recommendation: leave as ruled — the row and `ampere/backends/torch/__init__.py`'s module docstring describe it); W2.5 slice 3 — `use_realisation=True` on torch gains nothing, and `ampere_realised = 1` on gradient-free realised runs is a visible provenance change; `:no-index:` on the legacy API pages (W2.15).
4. **Branch triage** — `docs/design/harvest/branch_triage.md`'s *execution record* says the archival was approved and executed 2026-09-01 (tags pushed, 13 remote branches deleted, `small_silicates` kept), which contradicts the "awaits approval" wording in W0.7's status row and the previous handoff. Confirm, and the stale wording can be struck.
5. **Repository actions only Peter takes**: push master to origin and watch the first live CI run (a branch-protection rule naming the old `phase1-suites` job needs `suites`); delete the two stray local `worktree-agent-*` branches; run `pixi run gpu` in the `torch`/`jax` environments on a machine with an accelerator.
