...
**For Peter — what is still open.** One ruling from W3.1 slice 1: the multiprocessing start method for `ProcessExecutor` — `fork` today (works; jax/torch warn about forking a threaded runtime; Python 3.14 changes the platform default), `mp_context=` the per-call escape hatch; recommendation: default to `forkserver` on POSIX in slice 2 and record it. Otherwise only the repository actions he alone takes: push master to origin and watch the first live CI run (a branch-protection rule naming the old `phase1-suites` job needs `suites`); delete the stray local `worktree-agent-*` branches; run `pixi run gpu` in the `torch`/`jax` environments on a machine with an accelerator. Every ruling from the 2026-09-08 digest is recorded; W2.9's coverage study is scheduled inside W3.6.
