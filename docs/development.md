...
**W2.13 MERGED** at `0e7c97c`; **W2.4 slice 2 MERGED** at `a529129` (status row). **In flight (2026-09-07, night)**: **W2.5 slice 2** (`w2.5-jax-slice2`, reported complete at `48114a2`, reviewed on substance) is being **rebased by its agent onto `a529129`** with the reconciliation fixed by Fable: one `_vi.py` with both routes (master's pyro driver + a `_fit_numpyro`, master's guide names and provenance keys), master's `Engine.finish(log_likelihood_terms)` shape, one parametrised `test_vi.py`, both new conformance shapes kept (`CORRELATED_QUASISEP` + `PHOTOMETRIC`), both tracks' decision-log rows kept and §6's bullet reconciled. On its return: Fable re-review of the reconciliation, gates in all three environments, merge. Neither slice 2 changed `pixi.lock` (both chose celerite2, already a base dependency). Slice 2 scope as dispatched — prompts drafted in this session's scratchpad (`w2.4-slice2-prompt.md`, `w2.5-slice2-prompt.md`; re-derive from the W2.13/W2.4/W2.5 status rows if lost): quasiseparable library by measurement with the table as a decision-log row, the realised path widened (censoring, latent GPs, non-Gaussian families), VI via pyro/numpyro through `realise`, batching/device with the float32 opt-out via `provenance_config`, `conditional_loo` O(N) if the library allows, plus the carried fixes above. Both add to their extras and regenerate `pixi.lock` in a dedicated commit; expect the lockfile collision at the second merge.
