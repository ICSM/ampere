...
**In flight (2026-09-07, evening)**: **W2.13 dispatched** to one Opus agent (branch `w2.13-realisation`, base **`f74a053`** = tip of `w2.13-realisation-prototype`, which carries the prototype + the contract text: `inference.md` §10a, the twelve-decision row in the plan's §2, the `lowering.md` §0 pointer, the item in WORK_ITEMS.md). Peter accepted all twelve recommendations. On return: Fable review, run all three environments' gates, merge (the prototype branch's three commits come with it); then dispatch W2.4/W2.5 slice 2 in parallel from the merged tip, and W2.7/W2.8/W2.9 as capacity allows.
...
**Smaller rulings from W2.4** (all recorded in the W2.4 status row): `IcdfFallbackWarning` wants a home in `ampere.core.exceptions` beside `LoweringError` so both backends share one class (tiny §4 addition — proposed yes, fold into W2.13); `ParameterSet` should expose a public `evaluation_order()` so backends stop recomputing the topological order (proposed yes, fold into W2.13); `Likelihood.to_spec` records a dataclass solver's fields, so backend-specific solver configuration (dtype/device, later a float32 opt-in) cannot be dataclass fields without breaking cross-backend spec-hash agreement — needs a ruling before any GPU/precision work (proposed: a `provenance_config` hook on `GPSolver` recorded in `ampere_capabilities`/extra attrs, never in the spec hash); `pip install "ampere[torch]"` resolves to a CUDA wheel on Linux (~2 GB) — CI may want PyTorch's CPU index (your call).
