...
## ⚡ Pick up here — PHASE 3 IN PROGRESS (2026-09-09): W3.0–W3.5, W3.8 merged; W3.6 in flight; W3.4, W3.11, W3.12, W3.7, W3.10 queued

**Where the project is.** Phases 0–2 complete (spec frozen at `spec-v1.0`; three backends; M2 reached). **Phase 3, the SBI layer, is more than half landed.** Merged, in order: W3.0 (housekeeping), W3.9 (migration page), W3.1 slice 1 (`simulate_many`, the executor protocol, external simulators), W3.2 (`SBIEngine` over sbi 0.27), W3.1 slice 2 (native batched prediction and sampling, sharding hooks, `forkserver`), W3.3 (the coordinate–value–mask encoding, `EncodingLayout`, the masked set/transformer embeddings), W3.5 (trained-artefact caching, wired into the engine), W3.8 (non-native parts refused by default, opt-in without gradients). Every merged item's row in WORK_ITEMS.md's status table is the authoritative record of what landed, what was accepted at review, and what is carried. **Master is `9e4bc7d`+**, clean apart from the untracked paper script under `examples/examples_paper/` (leave it). Last fully recorded gates (after W3.3+W3.5, at `ed3eaaf`): **dev 1867/191, jax 2338/122, torch 2509/120, sbi 2574/55**, lint/format clean, pyrefly 0 errors in every environment. The W3.8 merge's four gates were queued at the merge (`<scratchpad>/gates-w38.summary`, session-local) — if unrecorded in W3.8's status row, re-run them one at a time. **Nothing is pushed to origin** (origin/master is still at the W1.3 handoff, `b8e585b`).
...
**In flight.** **W3.6** (Opus, branch `w3.6-calibration`, worktree under `.claude/worktrees/`): family D — `ampere.results.calibration` with SBC/TARP over the encoding for an `SBIEngine` posterior, the any-engine SBC loop, `plot_sbc_ranks`/`plot_coverage`, and the WStat low-count coverage study on `docs/source/wstat_comparison.rst`. It owns `ampere/results/*` (not `provenance.py`), `ampere/inference/_sbi.py`, the WStat example/page, `diagnostics.md`, `results.md`. If this session was cut off: `git branch --list 'w3.6*'`, `git worktree list`; a branch with commits is reviewed against W3.6's Accept line and merged `--no-ff` (gates dev + sbi on merged master; W3.6 may add an `emit()` extra-sample-stats hook — if so, its decision-log row text is in its report and must be logged at the merge); otherwise re-dispatch from the item text.

**Next, in order** (all ruled; item texts in WORK_ITEMS.md "Phase 3"; dispatch prompts are regenerated from the item text plus the standing paragraph below): after W3.6 merges — **W3.4** (Opus; TMNRE through sbi's own `NRE` + `RestrictedPrior` rounds, *no swyft* — ruled 2026-09-09; the design paragraph on the item is the scope), **W3.11** (Sonnet; set-embedding default width `max(2·free_size, 32)` and a masked-mean transformer readout), **W3.12** (Sonnet; `model_identity_hash` promoted into `provenance.py`, recorded on runs and training sets, `PROVENANCE_SCHEMA_VERSION` → 6, `append_training_set` refuses a model change — goes **alone or last**, every run's attrs change); all three touch `_sbi.py` or `results/`, so W3.4 ∥ W3.11 is fine only with one owner of `_sbi.py` (give it to W3.4; W3.11 appends after) — simplest is W3.11 → W3.4 → W3.12 in sequence, each ~one session. Then **W3.7** (Sonnet; the blocking `sbi` CI leg) and **W3.10** (Sonnet; automatic paging above the plot caps, not urgent). Phase 3 closes with a documentation pass like W2.15 (the SBI tutorial, `examples/sbi/` on the docs site, the deferred-list review) — draft that as W3.13 when W3.7 lands.
