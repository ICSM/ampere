...
## ⚡ Pick up here — PHASE 4 FINISHED 2026-09-13; **PHASE 5 APPROVED 2026-09-15** (WORK_ITEMS.md "Phase 5": W5.0–W5.19, D2–D8 ruled, D1 provisional, W5.20 proposed); **wave 1 (W5.0 ∥ W5.2) is ready to dispatch on Peter's go**; two open items below
...
**What a clean session does next.** (1) Read the 2026-09-15 rulings block below; two items are still open (D1's final ruling after the consequences analysis, and W5.20 which came out of it; Peter's word on the W4.7/W4.10 explanations). (2) **Dispatch wave 1 — W5.0 ∥ W5.2 — when Peter says go** (he approved the phase on 2026-09-15 but the go was not given in the same message), from the standing paragraph, Sonnet both, with file ownership: W5.0 owns `ampere/results/`, `ampere/inference/_vi.py`, `_dynesty.py`, `_sbi.py`'s attrs and `results.md`; W5.2 owns everything in its list and nothing in W5.0's. Then W5.20 if ruled in (Opus). (3) Gate each merged wave once in all four environments through the `flock`. The Phase 5 draft's inputs, for whoever re-reads it:
