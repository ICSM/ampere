...
## ⚡ Pick up here — PHASE 5 IN FLIGHT; **waves 1–2 merged and gated, W5.7 and W5.23 merged (W5.0, W5.2, W5.20, W5.4, W5.3, W5.5, W5.13, W5.6, W5.7, W5.23; the wave-3 merged-master gate green on `8d785e1`/`27e9bd1`: dev 2695/410, sbi 3744/94, torch 3614/224, jax 3434/233)**; **wave 3 (W5.7 ∥ W5.9) dispatched 2026-09-16 evening on Peter's go** (W5.7 first, W5.9 an hour later); **Interruption 2026-09-17 ~03:15: the session rate limit killed both agents (reset 03:20); both worktrees were clean — W5.9 at `743491b` (master merged in, the calibration comparison being reworked to rank the quantity the systematic corrupts), W5.22 at its base with no commits — and both were resumed from their transcripts at 03:35.** **IN FLIGHT (as of 2026-09-17 ~03:00): W5.22 (Sonnet, branch `w5.22-population-full-priorspec`, base `88528ef`, dispatched into the slot W5.23 freed; owns `results/provenance.py`, `results/population.py`, `results/training.py`, the population and provenance tests, the markers list in `pyproject.toml`, `results.md` §9/§13) and W5.9 (Opus, branch `w5.9-joint-noise-icm`; **finished 04:10 on 2026-09-17, reviewed, awaiting Peter's merge — see the block below**) and W5.23 (Sonnet, branch `w5.23-serve-artefact`, base `1f2d53c`; **merged at `05ff62f` on 2026-09-17**). Earlier: the W5.7 agent (Opus, branch `w5.7-warped-kernel`, base `a6a09c3`) dispatched 22:35 on 2026-09-16 into a fresh harness worktree; the W5.9 agent (Opus, branch `w5.9-joint-noise-icm`, same base) dispatched 23:40 the same evening; **W5.7 merged at `97f7662` on 2026-09-17** — if this session died, check `git worktree list` and `git branch --list 'w5.*'` for their branches, preserve any uncommitted worktree edits as a WIP commit, and resume each from its branch
...
**W5.9 REVIEWED 2026-09-17, READY FOR PETER'S MERGE** (branch `w5.9-joint-noise-icm`, head `09a312f`; the agent merged `w5.7-warped-kernel` and master `05ff62f`+ into it itself, so it is a clean fast-forward-style merge onto master `a4ad92e`; 33 files, +4268/−49). Fable's second pass: the rotated rescaled density and its leave-one-out terms re-derived, the rotation's orientation (`R Q` = `Qᵀ r` per sample) checked, the couplings read, the calibration study's mechanism checked against `1/√(1+ρ)`; re-run on the branch: dev 93 passed, jax 54 passed; no fixes needed. **Peter runs, from the main checkout**: `git merge --no-ff w5.9-joint-noise-icm` with the message below. Then the orchestrator pastes the rows with the merge hash, removes the worktree (`git worktree remove --force --force .claude/worktrees/agent-a8a99564e1fea3451`), **launches the merged-wave gate (all four environments, detached through the flock) covering W5.7 + W5.23 + W5.9**, and tells W5.22 (in flight) to merge master before reporting (a one-line conflict at the end of `pyproject.toml`'s markers list is expected; keep both).

*Merge message:*

```
Merge branch 'w5.9-joint-noise-icm'

W5.9: joint noise over a tuple of channels — the shared-grid intrinsic
coregionalisation model, K = B ⊗ K_x, solved exactly at O(N) by
diagonalising B and scoring T rescaled scalar problems through the bound
solver; declared on the DatasetCollection, lowered onto torch and jax so
NUTS samples B; astrometry the first customer, with the calibration
study that shows what a cross-channel systematic actually corrupts.
Opus-authored, Fable-reviewed as the second pass; no fixes needed.

Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01Y4vLKdyyMpn1e2MTqixBwM
```

*Status row (WORK_ITEMS.md, after W5.23's):*

| W5.9 | merged 2026-09-17 at `MERGE_HASH` (Opus-authored, Fable-reviewed as the second pass — the rotated, rescaled solve (`u_s = r̃_s/√λ_s` against `K_x + diag(σ²/λ_s)` with `−(N/2) log λ_s` added back, and `−½ log λ_s` per leave-one-out term) re-derived; the closed-form `RotationCoupling` eigendecomposition and the `CholeskyCoupling` fallback read; the calibration study's negative result checked against the `1/√(1+ρ)` prediction; the orchestrator's re-run on the branch: dev (conformance astrometry, `test_noise_joint`, `test_joint_pointwise`, `test_calibration`) 93 passed in 43 s, jax (the NUTS rows and conformance astrometry) 54 passed in 23 s; **no fixes needed at review**; merged by Peter; the agent merged `w5.7-warped-kernel` and then master `05ff62f`+ into its branch itself, cleanly; the agent's runs: dev conformance astrometry + `test_noise_joint` 83, torch 84 in 80 s, jax 84 in 25 s, dev `tests/core` 1649/34 and `tests/results` 510/4, torch `tests/backends` 588/3, jax 409/11, the `astrometry_full` row 1 in 903 s; lint/format/typecheck clean in all three; the agent was killed once by the session rate limit at ~03:15 and resumed from committed state (the resumed segment alone: 547 k tokens / 14 tool uses; the first segment's count was lost); the merged-wave gate to be recorded). Landed: `ampere.core.JointGaussianProcessNoise(kernel, solver, datasets=…, coupling=…)` with `JOINT = True` (a `Likelihood` refuses it by name; it is declared on `DatasetCollection(datasets, joint={label: noise})`); `ChannelCoupling` (a `Parameterised` supplying its own eigendecomposition through `eigen(resolved, xp=…)`, one implementation for numpy, torch and jax), `RotationCoupling(angle, log_variance_0, log_variance_1)` (closed-form eigenvectors; the parameters take the bijection their prior's support implies, not `Log`) and `CholeskyCoupling` (general `T`, numerical `eigh`); the bound solver decides once for all rotated outputs (`QuasisepGP` on an ordered 1-D grid is the O(N) path); `DatasetCollection.contribution_labels()`/`group_of()`, `group_log_likelihood`, `draw_group` (one correlated draw), `JOINT_DECOMPOSITION = "joint"` in `ampere.results`, the pointwise group storing the rotated outputs under the member labels; twins and a `_LoweredJointGroup` in each backend's `problem.py`, the native per-dataset draw refusing a joint problem by name; three refusals at composition — shared grid (exact axis equality), shared mask, one shared σ vector across channels — and a free kernel amplitude beside a free `B` refused as one degree of freedom twice; `examples/astrometry --joint` and `--sbc {joint,independent,rigid}` with the injected centroiding systematic; `docs/source/astrometry.rst` §10; `likelihoods.md` §7 (new subsection) and §15 item 11 rewritten, `inference.md` §4.9 (new) and §7, `results.md` §6; five conformance rows per fixture, `BackendCapabilities.joint_noise`, `tests/core/test_noise_joint.py`, `tests/inference/test_joint_noise_nuts.py`, `tests/results/test_joint_pointwise.py`, `tests/examples/test_astrometry_example.py` (the full row behind `astrometry_full`, a 12-refit sibling always on). Fixed on the way, inside scope: `replace_observations` rebuilt SBC replicas without the collection's joint groups; `inference/engine.py`'s two label checks and `results/emission.py`'s `log_likelihood` group assumed one term per dataset. Measured (48 refits per arm, 0.90 interval on `(pmra + pmdec)/√2`): joint 0.917 (KS p 0.235), independent GPs with the correct marginals 0.729 (p 0.021; `1/√(1+ρ)` with ρ = 0.86 predicts 0.77), rigid 0.479; pinned at a 0.80 floor and a 0.10 gap; NUTS on torch (81 s) and jax (16 s) recovers `trace(B)` in its 95 % interval; lowered vs contract density torch 0.0, jax 1.4e-14. **For Peter**: (1) unequal per-channel uncertainties are refused rather than solved densely — heterogeneous per-epoch errors are the norm in real astrometry, so a dense `B ⊗ K_x + D` fallback is the natural first slice of the follow-on; (2) the study holds `B` at the truth in both arms and gives the comparison arm the correct marginals, so the pinned claim is "given the noise you are entitled to know, the joint model is calibrated and two independent GPs are not" — fitting `B` is what `--joint` and the NUTS rows do; (3) `T > 2` is implemented but unit-tested only; polarimetry stays the second test. Carried: `replace_observations` also drops `shared=`/`shared_label=` (one line, a hierarchical SBC study would silently refit without its hyperparameters); `ampere.results.sbc` ranks posterior variables only — a `derived=` hook is a general need; a `summarise`-side helper reporting `B` rather than its bimodal angle parameterisation; the general LMC, mismatched grids and unequal uncertainties on the dense/reduced-rank follow-on; the arviz chain-length warning at tiny budgets (pre-existing). |

*Decision-log row (DEVELOPMENT_PLAN.md §2, after the W5.23 row):*

| Joint noise over a tuple of channels: the shared-grid ICM (W5.9) | **`likelihoods.md` §15's eleventh limitation is lifted for the case it is exact in: a `NoiseModel` may now span several datasets. `JointGaussianProcessNoise(kernel, solver, datasets=…, coupling=…)` is one correlated process over `T` channels of one model on a shared grid with `K = B ⊗ K_x`, declared on the `DatasetCollection` (`joint={label: noise}`) rather than on a `Likelihood`, which refuses a noise model declaring `JOINT = True` by name** (merged MERGE_DATE, `MERGE_HASH`; Opus-authored, Fable-reviewed). `B` is a `ChannelCoupling`, a `Parameterised` that supplies its own **eigendecomposition** — `RotationCoupling` (an angle and two log-variances, the full `T(T+1)/2` of a 2×2 PD matrix expressed as an error ellipse on the sky or a Q/U leakage angle, eigenvectors in closed form) and `CholeskyCoupling` (`B = L Lᵀ`, general, at the price of a numerical `eigh` whose gradient is ill-conditioned exactly where two eigenvalues coincide). It stays O(N) because `Qᵀ` decouples the channels: `T` scalar solves through the bound solver, handed the **rescaled** problem `u = r̃_s/√λ_s` against `K_x + diag(σ²/λ_s)` with `−(N/2) log λ_s` added back, which works for any kernel where rescaling a kernel tree's amplitude would not. **Three §4 consequences.** (a) `DatasetCollection.contributions` is no longer one term per dataset: a group contributes **one** term under its own label, in place of its members' — `inference.md` §4.9's `"joint"` decomposition, the same vocabulary widening `"mixed"` was, with `contribution_labels()` the API a caller reads instead of `tuple(collection)`; `ampere/inference/engine.py`'s two realisation-label checks and `ampere/results/emission.py`'s `log_likelihood` group are keyed by it. (b) `results.md` §6 gains `"joint"`: the pointwise group stores the group's **rotated outputs**, one per member label on the shared grid, because the channels are not independent and a leave-one-out conditional of one alone would condition on its sibling without saying so. (c) `DatasetCollection.draw_group` draws the channels **correlated**, and both backends' native per-dataset draw refuses a joint problem by name — an uncorrelated draw is a training set from a different model, which every marginal check passes. **Three restrictions, each refused by name at composition rather than approximated**: the channels must share one grid (exact axis equality), one mask, and one per-sample uncertainty — `Qᵀ ⊗ I` leaves `diag(σ_t²)` diagonal only where every `σ_t` is the same vector — and the kernel's amplitude may not be free beside a free `B`, since `B ⊗ (a²K̃) = (a²B) ⊗ K̃` makes them one degree of freedom twice (one fixed amplitude in a `Sum` pins the scale, so a sum's relative amplitudes stay fittable). The **general LMC** (`Σ_q B_q ⊗ k_q`), **mismatched grids** and **unequal per-channel uncertainties** stay together on the dense/reduced-rank follow-on. **A parameterisation symmetry, recorded rather than removed**: `(θ → θ+π/2, v₀ ↔ v₁)` is the same `B`, so `B` is identified and the three parameters only up to that relabelling — a bimodal angle marginal, the label switching a mixture model has; summarise `B`, or fix the angle. **A contract defect found by lowering it**: a coupling's parameters take the bijection their *prior's own support* implies, not the `Log` `_as_hyperparameter` imposes on every kernel hyperparameter — a log-variance is negative for every ordinary value, and `Log` made the whole unconstrained vector `NaN` while the density stayed finite. **A defect in `ampere.results.calibration` fixed in the same PR**: `replace_observations` rebuilt an SBC replica without the collection's joint groups, so every replicate was refitted with independent channels while its data still carried the cross-channel structure — and the study reported that as undercoverage, the very signal the model exists to remove (joint-arm coverage 0.55 → 1.00 once carried over). **The first customer is astrometry**, and the calibration study is worth reading for its negative result as much as its positive one: a cross-channel systematic does not bias either channel's own parameter, it **correlates their errors**, so both arms' *marginal* coverage is nominal and checking marginals alone would have passed the independent model; a shared *parameter* does not show it either, because the orbital phase is measured by both channels in **quadrature** and the two score directions are temporally orthogonal. What sees it is the projection along which the errors add — `(pmra + pmdec)/√2`, the direction of the proper motion — where the joint arm covers 0.917 at the 0.90 level and two independent GPs given each channel's *correct marginal* amplitude cover 0.729, close to the `1/√(1+ρ)` an omitted correlation of 0.86 predicts (0.77); the rigid arm covers 0.479. Polarimetry (a Stokes kind) remains the second test. Conformance green in all three environments; five new rows per registered backend. **Merged on Fable's second pass alone (the Codex quota ruling); Peter to rule on the dense fallback for unequal uncertainties.** |

**W5.23 merged by Peter at `05ff62f` (2026-09-17); rows pasted; worktree removed.** **W5.7 merged by Peter at `97f7662` (2026-09-17); status and decision-log rows pasted; the agent's worktree removed. The W5.9 agent was told to merge `w5.7-warped-kernel` into its branch before reporting.**
