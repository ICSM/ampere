...
## ⚡ Pick up here — PHASE 3 IN PROGRESS (2026-09-09): W3.0–W3.6, W3.8, W3.11 merged; W3.4 merged; W3.14 in flight; W3.12, W3.15, W3.7, W3.10 queued
...
**W3.4 merged** (see its status row; dev + sbi merged-master gates queued at the merge — `<scratchpad>/gates-w34.summary`; if unrecorded in W3.4's status row, re-run those two). **In flight.** **W3.14** (Opus, branch `w3.14-generative-forms`, from `89ab1b7`; core `sample()` for Poisson/Student-t/complex Gaussian with native twins; all four gates). *(Record of the W3.4 dispatch follows.)* **W3.4** (Opus, branch `w3.4-tmnre`, dispatched from the W3.11 merge): TMNRE through sbi's own `NRE` + `RestrictedPrior` rounds — `SBIEngine(method="tmnre", rounds=, marginals=, truncation_epsilon=)`, the truncation box per round in the attrs, the marginal posteriors emitted per the item's design paragraph; owns `ampere/inference/_sbi.py` (+ a new `_tmnre.py` if it prefers), `tests/inference/test_sbi.py`, `inference.md`/`results.md` amendments, `docs/source/ampere.inference.rst`. If cut off: `git branch --list 'w3.4*'`; review against its Accept line; merge with dev + sbi gates. (W3.11 merged before it at `531a518`; merged-master gates dev 1923/217, sbi 2655/70 green.)
