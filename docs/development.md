...
**Interruption record.** The session rate limit killed all four agents at about 09:00 on 2026-09-11 with their work committed (W4.0 six commits, all four gates run, two jax failures in its own new test; W4.6 four commits, dev+sbi green; W4.5 five commits, dev green, sbi/torch/jax queued; W4.1 three commits, dev green). All four were resumed by `SendMessage` at 09:30 with bounded-polling instructions (a single Bash call must stay under ten minutes, so agents poll gates in eight-minute foreground loops and repeat). If they are cut off again, resume the same way or re-dispatch from the branch.

**Merged so far (2026-09-13)**: **W4.5** `6a78a2b` (its own merged-master gates all green: dev 2144/269, sbi 2967/81, torch 2850/198, jax 2674/202), **W4.0** `c9e4015`, **W4.6** `b6d9298`, **W4.1** `eb7b702` + fix-up `03b6c57` (a units import lost between W4.5 and W4.1; pyrefly caught it). **The first wave is fully merged; the merged-master gates for the combined master at `03b6c57` were launched from the main checkout at 02:40 on 2026-09-13** (logs in the session scratchpad). If this sentence is still here, they were not recorded: run `flock /tmp/ampere-gate.lock pixi run -e <env> test-all` for dev, sbi, torch, jax on master and record the counts in the W4.0/W4.6/W4.1 status rows. The first wave's worktrees were removed after merging; the `worktree-agent-*` harness branches can be deleted (part C). **Second wave dispatched 2026-09-13 at about 03:00 from base `a649799`** (the merged-master dev gate green at 2312/284 was the go signal; sbi/torch/jax still running): `w4.2-complex-gp` (Opus), `w4.7-astropy-translations` (Sonnet), `w4.10-ci-path-gating` (Sonnet), `w4.11-sed-composition` (Sonnet), worktrees under `.claude/worktrees/`, file ownership in each prompt (W4.2: likelihood families + backends' family/GP modules; W4.7: the backends' `astropy.py` + an optional neutral helper; W4.10: `.github/`, `pyproject.toml`'s task table and dev deps, `pixi.lock`, the CI prose; W4.11: `examples/sed_composition/`, its page, `tutorials.rst`, `tests/examples`). All told to poll gates in bounded eight-minute loops and to commit early. A cut-off agent is resumed by `SendMessage` or re-dispatched from its branch. **Progress 2026-09-13**: the first wave's merged-master gates all green (recorded in W4.0's row); **W4.10 merged `d009e65`, W4.7 merged `2f62fba`** (dev environment re-installed for the lockfile change; quick checks clean); **W4.11 merged `9c520c1`** (its NUTS-tuning and examples-lint findings are in its status row and join the owed list); **W4.2 merged `4d93323`** — the second wave is fully merged; **its four merged-master gates are green** (dev 2380/308, sbi 3250/114, torch 3128/236, jax 2951/241; recorded in W4.2's row). **W4.3 merged `64284d5` + fix-up `169b672`** (its four merged-master gates green, recorded). **W4.4 merged `9cef25a`** (the jax binding verified at the merge). **W4.9 merged `318034e`** — the template claim held (no core change; four template gaps listed for W4.8). **The final wave's gates (W4.4 + W4.9) were launched from the main checkout at that merge** — record in W4.9's row; if this sentence survives a session, re-run them. **W4.8 dispatched from `318034e`** (Sonnet; the documentation pass that closes the phase: the template gaps, the astropy and kernel pages, the annotation sweep, the plan's Phase 4 landed summary is Fable's at the close). W4.4 was dispatched from `169b672` (Sonnet, alone; W4.3's open questions in its prompt: reuse `tests/backends/interferometry_fixtures.py`, no uniform disc as a fitted model on a modern backend, budget the GP arm, `layout="set"` for SBI, quote TARP not the KS p-value). Then W4.9 after W4.4, W4.8 last. The decision-log row of W4.3 leaves one question for Peter (the native surface's canonical spelling). **The second wave's merged-master gates run once, after W4.2 and W4.11 merge** (the 2026-09-13 token-economy rule in `docs/orchestration.md`: agents run targeted tests, the orchestrator gates the merged wave). **Then**: W4.3 after W4.2; W4.4 after W4.3 and W4.11; W4.9 after W4.4; W4.8 last. **Part C for Peter** gains the new branch-protection check names (W4.10's row). Each item's text in WORK_ITEMS.md is current. Rate-limit lesson: dispatch fewer Opus agents at once, and tell every agent to poll gates in bounded eight-minute foreground loops. File ownership is in each prompt and in the items file's "Ordering" paragraph; W4.1 and W4.5 both edit `core/likelihood.py` (families vs kernels) — expect a hand merge there and on `core/__init__.py` exports. Gates serialise through `flock /tmp/ampere-gate.lock`; each agent runs dev, sbi, torch, jax. **A clean session does, in order**: (1) `git worktree list` and `git branch --list 'w4.*'` to see what survived; an agent cut off mid-flight is resumed from its branch (its committed work stands; re-dispatch with the same prompt and "continue from branch X"); (2) review each finished branch against its Accept line with the `docs/development.md` checklist, run the merged-master gates for every environment the item names, merge, update the status table, remove the worktree (`git worktree remove --force --force`); (3) after W4.0 merges dispatch **W4.11** (Sonnet); after W4.5 and W4.1 merge dispatch **W4.2** (Opus); after W4.6, **W4.7** (Sonnet); after W4.1 + W4.2, **W4.3** (Opus); W4.10 (Sonnet) any time; (4) refresh this section at every merge and dispatch. The scratchpad script behind memo §7.1 is not in the repo; W4.11 recreates it from the memo.
