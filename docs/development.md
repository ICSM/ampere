...
**W5.22 REVIEWED 2026-09-17 09:45, READY FOR PETER'S MERGE — the last item before the session closes** (branch `w5.22-population-full-priorspec`, head `3babc5f`; master merged in twice by the agent, so the merge is clean; 8 files, +618/−60). Fable's review: the provenance attr, the three-way interim-prior resolution and the marker read; re-run on the branch 212 passed; no fixes needed. **Peter runs, from the main checkout**: `git merge --no-ff w5.22-population-full-priorspec` with the message below. Then the orchestrator pastes the rows with the merge hash, removes the last worktree (`git worktree remove --force --force .claude/worktrees/agent-a97623a82395db612`), and rewrites this head for the clean session. **The gate for W5.22 alone is not run before the close** (a docs-and-tests item plus one attr; its own dev/sbi rows ran green): the next session's first act is the four-environment gate on the then-master, recording its counts in W5.22's row.

*Merge message:*

```
Merge branch 'w5.22-population-full-priorspec'

W5.22: the population_full marker on the 200-object archive (a 50-object
reduced archive in the default gate), and each free parameter's prior
recorded in provenance (schema 8) so fit_population's interim_prior is
optional, verified when supplied, and refused by name when neither is
available. Sonnet-authored, Fable-reviewed; no fixes needed.

Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01Y4vLKdyyMpn1e2MTqixBwM
```

*Status row (WORK_ITEMS.md, after W5.9's):*

| W5.22 | merged 2026-09-17 at `MERGE_HASH` (Sonnet-authored, Fable-reviewed — `free_priors`, the schema-8 attr and the three-way `_resolve_interim_prior` (spec-hash mixture refused first; stored prior read back when none supplied; a supplied prior verified against the stored one; a hierarchical stored prior refused only when it is the sole source) read; the orchestrator's re-run on the branch: `test_results.py` + `test_training.py` + the provenance doctests 212 passed in 15 s; no fixes needed; merged by Peter; the agent merged master twice (the markers-list conflict with W5.9 resolved by keeping both); killed twice by the session rate limit and resumed from committed state (one uncommitted edit preserved as a WIP commit by the orchestrator, folded in); ≈ 409 k tokens / 298 tool uses over three segments; the agent's runs: dev `test_population.py` 15/2 in 516 s (was 11/1 in 710 s before the marker), `test_results.py` 177, `test_training.py` 31, sbi `test_population.py` 16/1 in 621 s; lint/format/typecheck clean; the next merged-master gate to be recorded). Landed: `PROVENANCE_SCHEMA_VERSION` 8 with `ampere_free_priors` (canonical JSON of `{name: {"kind": "prior_spec", **PriorSpec.to_dict()}}`, or `{"kind": "hierarchical", **HierarchicalPrior.to_dict()}`) from `ampere.results.provenance.free_priors`; `fit_population(interim_prior=None)` reads the stored marginal prior back, verifies a supplied one, and refuses by name a disagreement, a pre-schema-8 archive with no supplied prior, and a hierarchical stored prior with none supplied; the `population_full` marker (registered, skipped by default through the new `tests/results/conftest.py`) on the 200-object archive, with a 50-object reduced archive shared by every other row (`TestTheReducedObjectsFit` pins the timing guard there); `results.md` §9 (the schema-8 paragraph) and §13 item 16 amended; two schema-version literals updated in `test_results.py` (`>= 7`) and `test_training.py` (`8`). **For Peter**: the reduced archive is 50 objects (the item text said 20; the dispatch offered the existing 50-object fixture); `append_training_set` does not gate on `ampere_free_priors` (only `fit_population` reads it); a stored hierarchical prior is refused rather than marginalised — confirm as the long-term answer. Carried: `tests/results/test_population.py` still costs ≈ 8.6 min in dev and ≈ 10.3 in sbi in the default gate — the 50 single-object fits and the 2000-step population emcee, not the archive size, dominate; a second reduction (fewer steps, or the archive built once and cached across the module) is a Sonnet filler; the item text's `tests/results/test_provenance*.py` does not exist (the schema rows live in `test_results.py`). |

*Decision-log row (DEVELOPMENT_PLAN.md §2, after the W5.9 row):*

| Free-parameter priors promoted to provenance; `fit_population`'s `interim_prior` optional (W5.22) | **`PROVENANCE_SCHEMA_VERSION` moves 7 → 8: every run records `ampere_free_priors`, canonical JSON of `{parameter name: PriorSpec.to_dict()}` for each free parameter (`ampere.results.provenance.free_priors`) — an ordinary declared prior tagged `{"kind": "prior_spec", ...}`, a `HierarchicalPrior` member tagged `{"kind": "hierarchical", ...}` through its own `to_dict()`** (merged MERGE_DATE, `MERGE_HASH`; Sonnet-authored, Fable-reviewed; ruled by Peter 2026-09-16 on W5.13's proposals). `ampere.results.population.fit_population`'s `interim_prior` becomes optional: omitted, the named parameter's stored prior (agreed across every input run sharing `ampere_spec_hash`, checked rather than assumed) is read back and used; supplied, it is checked against the stored one and a disagreement is refused by name; a run with no stored prior (schema < 8) and no supplied prior is refused by name, on `append_training_set`'s W3.12 "refused rather than guessed" precedent; a stored `HierarchicalPrior` is refused only when it is the sole source of the interim prior. `append_training_set`'s own invalidation (spec and model hashes) is untouched. `results.md` §9 and §13 item 16 amended. `tests/results/test_population.py`'s 200-object archive gains a `population_full` marker (skipped by default through `tests/results/conftest.py`, `image_full`'s pattern); every other row in that module shares a 50-object reduced archive. |

**GATE RECORDED: the merged-wave gate (W5.7 + W5.23 + W5.9) finished 06:58 on 2026-09-17, all green — dev 2818/422, sbi 3891/95, torch 3757/229, jax 3577/238 — and the counts are in the three rows. Interruption 2 (2026-09-17 ~08:00): the session rate limit killed W5.22 again (reset 08:30) and the harness's low-memory guard killed the orchestrator's two gate waiters (the detached gate itself survived, as designed); W5.22's one uncommitted edit (`tests/results/test_results.py`) was preserved as a WIP commit on its branch and the agent resumed at 09:15.** **W5.9 merged by Peter at `8e10b55` (2026-09-17); rows pasted; worktree removed; his rulings recorded (heteroscedastic channels essential → W5.24 drafted; the study design accepted). The merged-wave gate (W5.7 + W5.23 + W5.9) launched detached on `8e10b55` at 04:13 on 2026-09-17 — legs dev → sbi → torch → jax, one summary line per leg in `/tmp/claude-1000/-home-pscicluna-GitHub-ampere/97357ead-1dac-41e5-aba5-b74825189e16/scratchpad/wave3-gate.log` (full output beside it as `.log.<env>`; `=== ALL DONE` closes it); record the four counts in the W5.7, W5.23 and W5.9 rows. Baseline, the previous gate on `8d785e1`: dev 2695/410, sbi 3744/94, torch 3614/224, jax 3434/233.** **Peter's plan (2026-09-17, ~04:30): after W5.22 finishes and merges, clean up to start a new session** — so the next steps are: W5.22's review and merge (it must merge master first; the markers-list conflict is expected), record the gate, remove W5.22's worktree, delete the merged `w5.*` and harness branches if Peter wants, and rewrite this head for the clean session (wave 4: W5.10 (+ W5.11) ∥ W5.8; W5.21 now unblocked; W5.24 new). **W5.23 merged by Peter at `05ff62f` (2026-09-17); rows pasted; worktree removed.** **W5.7 merged by Peter at `97f7662` (2026-09-17); status and decision-log rows pasted; the agent's worktree removed. The W5.9 agent was told to merge `w5.7-warped-kernel` into its branch before reporting.**
