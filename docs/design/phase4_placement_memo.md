...
| The native twins | `ampere/backends/torch/instrument.py`, `ampere/backends/jax/instrument.py` | **no registry**: the same class names in parallel modules, a duck-typed `apply_flux` surface the realised problem checks (`backends/torch/problem.py:327`, `backends/jax/problem.py:285`), beside the model's own `native_flux`/`native_grid` pair (canonical since W5.20; `flux`/`grid` is the legacy alias, and offering both is refused as ambiguous), and the `BACKEND` flag that `foreign_parts` checks structurally (`core/dataset.py:409`) |
