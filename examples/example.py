...
if __name__ == "__main__":
    ...
    """ Define the model """
    modwaves = 10 ** np.linspace(0.0, 2.0, 2000)
    # print(modwaves)
    # print(10**modwaves)
    # exit()
    Tbb = 250.0
    area = 10.0 * u.au.to(u.m) ** 2  # .value)^2
    print(Tbb, np.log10(area), 0.0, 3.0)
    # exit()
    TestData = SingleModifiedBlackBody(
        modwaves,  # irs.wavelength, #modwaves,flatprior=True,
        normWave=20.0,
        sigmaNormWave=100.0,
        redshift=False,
    )
    ...
    TestData(t=Tbb, scale=np.log10(area), index=0.0, dist=3.0)
    for i in irs:
        # print(i)
        ...
        # print(i)
        # irs.value = TestData.modelFlux #spectres(irs.wavelength, modwaves, TestData.modelFlux)
        # #['flux'].data = TestData.modelFlux
    ...
    filts = filterLib.load_filters(filters, interp=True, lamb=modwaves * get_unit("micron"))
    ...
    irs.append(phot)
    # exit()
    # model = PowerLawAGN(irs.wavelengths) #We should probably define a wavelength grid separate
    # from the data wavelengths for a number of reasons, primarily because the photometry needs
    # an oversampled spectrum to compute synthetic photometry

    model = SingleModifiedBlackBody(
        modwaves,  # irs.wavelength,flatprior=True,
        normWave=20.0,
        sigmaNormWave=100.0,
        redshift=False,
        lims=np.array([[150.0, 400.0], [15.0, 35.0], [-2.0, 2.0], [1.0, 5.0]]),
    )

    """ and the extinction law (if necessary) (could be moved inside model
    as only some models will need this) """
    # ext = CCMExtinctionLaw()

    # model.extinction = ext #combination of multiple different models is
    # something we need to consider how to implement.
    ...
    opt = EmceeSearch(
        ...
        data=irs,
        nwalkers=100,
        # introspection is used inside the optimiser to determine the number
        # of parameters that each part of the model has
    )
    ...
    """ Run it """
    pos = [
        [  # noqa: RUF005 -- list + ndarray is elementwise addition (jitter), not concatenation
            250.0,
            23.0,
            0.0,
            3.0,
        ...
        + np.random.randn(int(opt.npars))
        for j in range(opt.nwalkers)
    ]
    print(pos[0])
