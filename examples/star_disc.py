...
# Photometry
photometry = data.Photometry.fromFile(
    libdir + "../examples/star_disc/HD105_SED.vot",
    format="votable",
...
# IRS spectrum
irs_spec = data.Spectrum.fromFile(
    libdir + "../examples/star_disc/cassis_yaaar_spcfw_5295616t.fits",
...
fbol_1l1p = (
    (fbol_1l1p / (4 * np.pi * (1 * units.pc) ** 2)).to(units.erg / units.s / units.cm / units.cm)
).value
...
print(path)
download_models = (
    False  # Should be True if this is the first time you're running this script, others False
)
if download_models:
    ...
    ranges = [
        [5000.0, 8000.0],  # T
        [4.0, 5.0],  # log g
        [0.0, 0.5],  # We'll stick with solar metallicity here
    ]
...
if recreate_hdf5:
    creator = HDF5Creator(
        grid, libdir + "../examples/star_disc/ampere_test_grid.hdf5", ranges=ranges
    )
...
# Now we have a spectral emulator that can be called to interpolate to arbitrary stellar
# parameters within the bounds of the grid it was trained on.

# Use the Gaia DR2 distance to HD 105 as a fixed value, which should constrain the degeneracy
# between radius and luminosity
parallax = 25.7534
...
# Now to convert to a more useful unit, we calculate the bolometric flux of the model, and we
# will rescale it to the desired luminosity
...
star_spec = data.Spectrum(
    specwaves, spec0, unc0, "um", "Jy", calUnc=0.10, scaleLengthPrior=0.01
...
# self.parLabels = ['lstar','tstar','log_g','[fe/h]','Adust','tdust','lam0','beta'] + noise
# parameters
guess = [
    1.0,
    5900.0,
