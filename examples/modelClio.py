...
# $F_\lambda \propto \sum_{j=1}^{\{2,3\}} (( \sum_{i=1}^8 \alpha_i \kappa_i ) \times \beta_j
# B_{\lambda} (T_j)) + \sum_{k=1}^{\{2,3\}} \beta_k B_{\lambda}(T_k)$
...
if __name__ == "__main__":
    ...
    #    spec = Spectrum(wavelength=sws['Wavelength'].data,value=sws['Flux'].data,
    #    uncertainty=sws['Uncertainty'].data,fluxUnits='Jy',bandUnits='um')

    """ Define the model """

    modwaves = 10 ** np.linspace(0.3, 2.3, 2000)  # 2-200 um spectrum w/ 2000 data points

    Tmin = 100.0  # minimum temperature allowed in fit
    Tmax = 1000.0  # maximum temperature allowed in fit
    ...
    mdl = DualBlackBodyDust(
        modwaves,
        normWave=1.0,
        sigmaNormWave=1.0,
        opacityFileList=opacities,
        lims=np.array([[0, 1e6], [-100, 100], [-10, 10], [0, np.inf]]),
    )
    ...
    opt = EmceeSearch(
        ...
        data=[spec],
        nwalkers=100,
        # introspection is used inside the optimiser to determine the number of parameters
        # that each part of the model has
    )
