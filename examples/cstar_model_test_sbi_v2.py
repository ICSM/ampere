...
limits = np.asarray(
    [[-10, -6], [-2, 2], [2, 4], [-2, 2], [1, 3], [1e3, 1e4], [0.0, 1.0], [0.0, 1.0]]
)
...
model = ampere.models.Hyperion.HyperionCStarRTModel(
    np.logspace(np.log10(0.2), np.log10(200), 1000, base=10),
    nproc=70,
    lims=limits,
    parLabels=labels,
)
...
p = data.Photometry.fromFile(
    "Observed_SED.vot", format="votable", libName=libdir + "ampere_allfilters.hd5"
)

p.reloadFilters(np.logspace(np.log10(0.2), np.log10(200), 1000, base=10))

s = data.Spectrum.fromFile(
    "SPEC_OGLE_CAGB_IRS.csv",
    "User-Defined",
    filetype="text",
    keywords={
        "waveCol": "wavelength",
...
# t = data.Spectrum.fromFile('OGLE_LMC_LPV_28579_photosphere_interpolated.vot', 'User-Defined',
#     filetype = 'votable', keywords = {'waveCol': 'LAMINTERP', 'fluxCol': 'SPECINTERP',
#     'waveUnit': 'micron', 'fluxUnit': 'Jy'})

dataset = [p]
for sp in s:
    dataset.append(sp)

optimizer_sbi = SBI_SNPE(
    model=model,
    data=dataset,
    name="Cstar_test_sbi_v2",
    namestyle="short",
    n_prior_norm_samples=100000,
    get_prior_bounds=False,
    n_rounds=2,
)
