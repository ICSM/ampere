...
names = np.array(
    ["am. oliv.", "forst.", "calcite", "ice", "diopside", "c-enst.", "dolomite", "iron"]
)
n0 = np.array(
    [
        (
...
r0cubed = (
    4.7e-2
    * msun
    / (math.pi / (3 - p) * ((30 / 60) ** (-4) - 1) * (4 / 3) * math.pi * a**3 * 3.71 * 3.9e-2)
)  # Kemper et al, 2002, using cold amorphous olivine
...
r0cubed = (
    6.1e-6
    * msun
    / (math.pi / (3 - p) * ((100 / 118) ** (-4) - 1) * (4 / 3) * math.pi * a**3 * 3.71 * 1.2e-4)
)  # Kemper et al, 2002, using cold amorphous olivine
...
# (3.14*(1e15)**3)/2.5 * ((27.83/61.06)**(-4) -1) * (4/3) * 3.14 * (0.1e-4)**3 * rho_d * n0 in
# c.g.s units


# another option is to calibrate the cold dust mass to what is derived in our 2002 paper for the
# total cold dust mass: 0.050 Msun
