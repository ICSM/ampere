...
class SpectrumNGC6302(Model):
    ...
    def __init__(
        ...
        if lims is None:
            ...
            # indexp = theta[20] 0, 1;
            self.lims[:, 0] = -6.0
            self.lims[11:13, 0] = 10.0
            self.lims[11:13, 1] = 80.0
            self.lims[13:15, 0] = 80.0
            self.lims[13:15, 1] = 180.0
        #            self.lims[15, 0] = 0.
        #            self.lims[15, 1] = 1.
        else:
            self.lims = lims
...
if __name__ == "__main__":
    ...
    )  # added extra keyword calUnc with a small value, and scalelengthPrior to restrict cov.
    # scale length
    ...
    # Ampere exposes access to emcee's moves interface. This can be useful if the posterior
    # turns out to not be well behaved - the default move only deals well with posteriors that
    # are monomodal and approximately Gaussian. Here's an example that usually deals a bit
    # better with posteriors that don't meet these criteria:
    m = [
        (moves.DEMove(), 0.8),
