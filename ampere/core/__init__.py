"""Backend-neutral core contracts for ampere v2.

Everything in this package is pure numpy/scipy/astropy.units/stdlib. It never
imports torch, jax, paramax or any other optional dependency, lazily or
otherwise (``architecture.md`` §3-4), because it is the shared vocabulary the
reference, torch and jax backends all implement.

Currently landed: the parameter and prior contract (W1.3,
``docs/design/contracts/parameters.md``), the ModelResult schema (W1.4,
``docs/design/contracts/results_schema.md``) and the likelihood/noise-model
contract (W1.6, ``docs/design/contracts/likelihoods.md``). The remaining §4
contracts — ``transform``, ``dataset``, ``astropy_compat`` — arrive with W1.5
and W1.7.
"""

from __future__ import annotations

from .exceptions import (
    AmpereError,
    ChannelError,
    ContractError,
    LikelihoodError,
    OptionalDependencyError,
    ParameterError,
    SchemaError,
    TyingError,
)
from .likelihood import (
    DTYPE,
    CauchyFamily,
    Censoring,
    ComplexGaussianFamily,
    DenseGP,
    GaussianFamily,
    GaussianProcessNoise,
    GPConditional,
    GPSolver,
    IndependentNoise,
    InducingPointGP,
    Kernel,
    KernelSpec,
    LatentDeclaration,
    Likelihood,
    LikelihoodFamily,
    LimitKind,
    Marginalisation,
    Matern32,
    NoiseModel,
    NoiseParams,
    PoissonFamily,
    QuasisepGP,
    RiceFamily,
    SquaredExponential,
    StructuredGridGP,
    StudentTFamily,
    VecchiaGP,
    VonMisesFamily,
    WindowedSparseGP,
    family_named,
    latent_parameter,
    list_families,
    register_family,
)
from .parameter import (
    SEPARATOR,
    Bijection,
    Binding,
    Buffer,
    BufferSet,
    HierarchicalPrior,
    Identity,
    Log,
    Logit,
    Parameter,
    Parameterised,
    ParameterMapping,
    ParameterSet,
    Plate,
    Prior,
    PriorSpec,
    Tie,
    default_bijection_for,
    describe_prior,
    log_density,
    prior_from_spec,
)
from .results_schema import (
    DEFAULT_CHANNEL,
    REGULARITY_RTOL,
    Axis,
    AxisSpec,
    Cube,
    FunctionSamples,
    Image,
    Layout,
    ModelResult,
    Order,
    PhotometricPoints,
    Spectrum,
    TimeSeries,
    VisibilitySet,
)

__all__ = [
    "DEFAULT_CHANNEL",
    "DTYPE",
    "REGULARITY_RTOL",
    "SEPARATOR",
    "AmpereError",
    "Axis",
    "AxisSpec",
    "Bijection",
    "Binding",
    "Buffer",
    "BufferSet",
    "CauchyFamily",
    "Censoring",
    "ChannelError",
    "ComplexGaussianFamily",
    "ContractError",
    "Cube",
    "DenseGP",
    "FunctionSamples",
    "GPConditional",
    "GPSolver",
    "GaussianFamily",
    "GaussianProcessNoise",
    "HierarchicalPrior",
    "Identity",
    "Image",
    "IndependentNoise",
    "InducingPointGP",
    "Kernel",
    "KernelSpec",
    "LatentDeclaration",
    "Layout",
    "Likelihood",
    "LikelihoodError",
    "LikelihoodFamily",
    "LimitKind",
    "Log",
    "Logit",
    "Marginalisation",
    "Matern32",
    "ModelResult",
    "NoiseModel",
    "NoiseParams",
    "OptionalDependencyError",
    "Order",
    "Parameter",
    "ParameterError",
    "ParameterMapping",
    "ParameterSet",
    "Parameterised",
    "PhotometricPoints",
    "Plate",
    "PoissonFamily",
    "Prior",
    "PriorSpec",
    "QuasisepGP",
    "RiceFamily",
    "SchemaError",
    "Spectrum",
    "SquaredExponential",
    "StructuredGridGP",
    "StudentTFamily",
    "Tie",
    "TimeSeries",
    "TyingError",
    "VecchiaGP",
    "VisibilitySet",
    "VonMisesFamily",
    "WindowedSparseGP",
    "default_bijection_for",
    "describe_prior",
    "family_named",
    "latent_parameter",
    "list_families",
    "log_density",
    "prior_from_spec",
    "register_family",
]
