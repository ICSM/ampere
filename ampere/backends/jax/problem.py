...
surfaces — ``model.native_flux``, ``step.apply_flux``, ``noise.sigma_jax``,
...
class _LoweredDataset:
    ...
    def __init__(self, problem: FittingProblem, label: str) -> None:
        ...
        raw = np.asarray(observed.values).ravel()
        ...
        self.uncertainty = (
            ...
            if observed.uncertainty is None
                ...
                np.asarray(observed.uncertainty, dtype=float).ravel()[self.retain],
                dtype=jnp.float64,
            )
        )
